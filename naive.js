...
// consts
const PLUGIN_NAME = 'gulp-naive';

function expandedNodeUpdater( node, isLeft, _width ) {
...
function updateContour( path, curviness ) {
	...
	glyph.contours.forEach(function( contour, i ) {
		...
		contour.nodes.forEach(function( node, j ) {
			...
			if ( !node.src.expandedTo ) {
				...
				_.assign( node.src,
					autoExpandableNodeSrc( node, i, j, true) );
...
function naive(/*prefixText*/) {

	// creating a stream through which each file will pass
	var stream = through.obj(function(file, enc, cb) {
		const sandbox = { exports: { glyphs: {} } };

		const font = JSON.parse(file.contents.toString());
