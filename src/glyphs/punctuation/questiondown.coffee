			...
			nodes:
				...
				2:
					x: Math.max(
						...
						contours[0].nodes[0].expandedTo[1].x + 0.25 * ( 120 / defaultThickness ) * thickness + 10
					)
				...
				3:
					...
					y: ( (400 + ( (45 / defaultThickness) * thickness )) / 750 ) * capHeight - (19)
					...
					dirOut: Math.max(
						...
						Math.min(
							35,
							(( 35 / defaultThickness ) * thickness) / 750 * capHeight
						)
					) / 180 * Math.PI
					typeIn: 'smooth'
					tensionIn: 1.2
					...
					expand:
						...
						distr: 0.75
				4:
					x: contours[0].nodes[1].x - ( 8 / defaultThickness ) * thickness
